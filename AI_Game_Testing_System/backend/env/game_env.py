...
from backend.env.screen_capture import ScreenCapture
from backend.env.action_executor import ActionExecutor
from backend.env.state_processor import StateProcessor
from backend.env.reward_engine import RewardEngine
from backend.analytics.coverage_tracker import CoverageTracker
from backend.analytics.crash_detector import CrashDetector
from backend.core.config import IMG_HEIGHT, IMG_WIDTH, FRAME_STACK_SIZE
...
class GameEnv(gym.Env):
    ...
    def __init__(self, config=None):
        ...
        self.observation_space = spaces.Box(
            low=0.0, high=1.0, 
            shape=(FRAME_STACK_SIZE, IMG_HEIGHT, IMG_WIDTH), 
            dtype=np.float32
        )
