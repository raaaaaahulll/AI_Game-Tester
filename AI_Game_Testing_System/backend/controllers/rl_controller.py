...
class RLController:
    ...
    def __init__(self):
        ...
        self._selected_window_hwnd: Optional[int] = None
        self._focus_check_thread: Optional[threading.Thread] = None
    ...
    def _training_loop(self, genre: str) -> None:
        ...
        try:
            ...
            # Start window focus monitoring if window is selected
            ...
            metrics_collector.update("error_log", error_message)
            # Log the error details for debugging
            logger.error(f"Training loop error details: {error_message}", exc_info=True)
        finally:
            ...
            with self._lock:
                ...
                # Ensure thread reference is cleared if thread is dead
                if self.thread and not self.thread.is_alive():
                    self.thread = None

    def _start_focus_monitoring(self) -> None:
        """Start monitoring window focus status."""
        if self._focus_check_thread and self._focus_check_thread.is_alive():
            return
        
        def check_focus_loop():
            """Periodically check if selected window is focused."""
            while not self.stop_event.is_set() and self._selected_window_hwnd is not None:
                try:
                    is_focused = windows_service.is_window_focused(self._selected_window_hwnd)
                    metrics_collector.update("window_focused", is_focused)
        ...
        self._focus_check_thread = threading.Thread(
            target=check_focus_loop,
            daemon=True,
            name="FocusMonitorThread"
        )
        self._focus_check_thread.start()
        logger.info("Started window focus monitoring")
    
    def _stop_focus_monitoring(self) -> None:
        """Stop monitoring window focus status."""
        if self._focus_check_thread:
            # Thread will stop on next iteration due to stop_event
            self._focus_check_thread = None
            metrics_collector.update("window_focused", False)
            logger.info("Stopped window focus monitoring")
    
    def start_test(self, genre: str, window_hwnd: Optional[int] = None) -> Tuple[bool, str]:
        """
        Start a new testing session.
        
        Args:
            genre: Game genre to test (must be valid)
            window_hwnd: Optional window handle ID to track
        ...
        with self._lock:
            ...
            # Store selected window handle
            self._selected_window_hwnd = window_hwnd
            if window_hwnd is not None:
                logger.info(f"Tracking window with hwnd: {window_hwnd}")
            
            # Start training thread
            self.stop_event.clear()
            self._current_genre = genre_lower
            
            # Clear any previous error status and reset to initializing
            metrics_collector.update("status", "Initializing")
            metrics_collector.update("window_focused", False)  # Initialize focus status
    ...
    def stop_test(self) -> Tuple[bool, str]:
        ...
        with self._lock:
            ...
            # Signal stop (this will also stop focus monitoring)
